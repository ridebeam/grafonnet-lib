  ...
  panel.new(title='Number of trips end at ' + cityName, time_shift='5m')
    .addTargets([
      target.target(
        ...
        query="SELECT $timeSeries AS t, countMerge(count) as c FROM $table WHERE $timeFilter and event_name = 'TRIP_END_SUCCESS' and city_id='"+cityId+"' GROUP BY t ORDER BY t ASC",
        formattedQuery="SELECT $timeSeries AS t, countMerge(count) as c FROM $table WHERE $timeFilter and event_name = 'TRIP_END_SUCCESS' and city_id='"+cityId+"' GROUP BY t ORDER BY t ASC",
...
grafana.dashboard.new(
  'Trips End By City All',
  uid='jwebb_trips_end_by_city_all',
  refresh='5m',
