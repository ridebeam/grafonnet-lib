  ...
  panel.new(title='Number of trips end at ' + cityName, time_shift='5m')
    .addTargets([
      target.target(
        ...
        query="SELECT $timeSeries as t, sum(count), max(lowerbound_2Z_today), max(median_wow) FROM $table WHERE $timeFilter AND event_name = 'TRIP_END_SUCCESS' AND city_id='"+cityId+"' GROUP BY t ORDER BY t",
