    ...
    alerts: [
      ...
      },
      {
        ...
        custom: {
          name: 'CrashLoopBackOff Count',
          query: |||
            increase(kube_pod_container_status_restarts_total{cluster="core-sg",exported_namespace=~"production",pod!~".*cdc.*|.*dbt.*|malaysia.*|korea.*|deploymentimagesample.*|standard.*|abortio.*|automator.*|flink-vehicle-snapshot-.*|experiment.*|analytics-watchdog.*"}[4m]) > 0
          |||,
          alias: '{{pod}}',
        },
        // 1 times
