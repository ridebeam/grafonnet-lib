    ...
    alerts: [
      ...
      },
      {
        ...
        custom: {
          ...
          query: '(sum(rate(promo_code_get_claimable_failed_total{namespace="production"}[30m]) OR vector(0))) / sum(rate(promo_code_get_claimable_attempt_total{namespace="production"}[30m]) > 0) * 100',
      ...
      },
      {
        ...
        custom: {
          ...
          query: '(sum(rate(promo_code_get_user_privileges_failed_total{namespace="production"}[30m]) OR vector(0))) / sum(rate(promo_code_get_user_privileges_attempt_total{namespace="production"}[30m]) > 0) * 100',
