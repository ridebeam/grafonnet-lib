    ...
    alerts: [
      ...
      },
      {
        ...
        custom: {
          ...
          query: '(sum(rate(promo_code_get_user_privileges_failed_total{namespace="production"}[2h]) OR vector(0))) / sum(rate(promo_code_get_user_privileges_attempt_total{namespace="production"}[2h]) > 0) * 100',
