    ...
    alerts: [
      {
        ...
        custom: {
          ...
          query: '(sum(rate(unet_add_ride_error{namespace="production"}[1m])) OR vector(0)) / sum(rate(unet_add_ride_attempt{namespace="production"}[1m]) > 0) * 100',
      ...
      },
      {
        ...
        custom: {
          ...
          query: '(sum(rate(unet_add_location_error{namespace="production"}[1m]))  OR vector(0)) / sum(rate(unet_add_location_attempt{namespace="production"}[1m]) > 0) * 100',
      ...
      },
      {
        ...
        custom: {
          ...
          query: '(sum(rate(unet_vehicle_status_update_error{namespace="production"}[1m])) OR vector(0)) / sum(rate(unet_vehicle_status_update_attempt{namespace="production"}[1m]) > 0) * 100',
    ...
    alerts: [
      {
        ...
        format: 's',
        threshold: 180,
        message: 'unet kafka consuming vehicle events lag > 3m',
      },
      {
        ...
        format: 's',
        threshold: 180,
        message: 'unet kafka consuming trip events lag > 3m',
      },
    ],
  },
  {
    ...
    alerts: [
      ...
      },
      {
        ...
        custom: {
          ...
          query: '(sum(rate(unet_get_user_info_error{namespace="production"}[1m])) OR vector(0)) / sum(rate(unet_add_ride_attempt{namespace="production"}[1m]) > 0) * 100',
