    ...
    alerts: [
      ...
      },
      {
        ...
        custom: {
          ...
          query: '(sum(rate(add_member_failed{namespace="production",priority="low"}[30m])) OR vector(0)) / sum(rate(add_member_attempt{namespace="production"}[30m]) > 0) * 100',
          alias: 'error with add member',
          intervalFactor: 2,
        },
        threshold: 40,
        thresholdType: 'gt',
        queryTimeStart: '30m',
        message: 'Error ratio of add member above 40%',
        noDataState: 'ok',
      },
      {
